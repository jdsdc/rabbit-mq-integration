package dev.coffe.springintegration.rabbitmqintegration;
...
import static dev.coffe.springintegration.rabbitmqintegration.RabbitMqIntegrationApplication.*;
...
class RabbitConfiguration {
    ...
    public RabbitConfiguration(ObjectMapper objectMapper) {
    ...
    @Bean
    ApplicationRunner basicProducer(AmqpTemplate rabbitTemplate) {
        return args -> {
            ...
            var message = MessageBuilder
                    ...
                    .build();
            rabbitTemplate.send(BASIC_REQUESTS_NAME, BASIC_REQUESTS_NAME, message);
        };
    }

    @RabbitListener(queues = BASIC_REQUESTS_NAME)
    public void basicConsumer(@Headers Map<String, Object> headers, @Payload Message payload) throws Exception {
    ...
    ApplicationRunner integrationRunner() {
        return args -> {
            integrationMessageChannel()
                    .send(org.springframework.messaging.support.MessageBuilder.withPayload(payload("Integration"))
                            .build());
        };
    }
...
class InfrastructureConfiguration {
    ...
    InitializingBean initializeRabbitMqBroker(AmqpAdmin admin) {
        return () -> Set
                .of(BASIC_REQUESTS_NAME, INTEGRATION_REQUESTS_NAME)
                .forEach(name -> define(admin, name));
    }

    private static Queue define(AmqpAdmin admin,
                                String name) {
        var q = QueueBuilder
                .durable(name)
                .build();
